...
def call(env, mavenCommand) {
    ...
    final int exitCode = sh(returnStdout: true, script: $/CURRENT_UID=`id -u` CURRENT_GID=`id -g` MVN_CMD='$mavenCommand' docker-compose -f docker-compose.yml --no-ansi -p $composeId up mvn; echo '\"$\"'/$).trim()
