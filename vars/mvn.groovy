...
def call(env, mavenCommand) {
    ...
    final int exitCode = sh(returnStdout: true, script: "CURRENT_UID=`id -u` CURRENT_GID=`id -g` docker run --rm mvn mvn -B -Duser.home=/tmp -s /build/settings.xml '$mavenCommand'; '$cmd'").trim()
